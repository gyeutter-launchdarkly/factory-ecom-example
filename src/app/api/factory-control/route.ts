...
import { demoProfile, scenarioBelongsToProfile } from '@/lib/demo-profile';
...
const ACTIONS = ['reset', 'run', 'replay', 'clear-history'] as const;
...
export async function GET(request: Request) {
  ...
  const scenarios =
    (await readJson<{ key: string; title: string }[]>(join(CONTROL_DIR, 'scenarios.json'))) ?? [];
  const profile = await demoProfile();

  return NextResponse.json({
    available: await watcherAlive(),
    busy: !!beat?.busy,
    scenarios: scenarios.filter((scenario) => scenarioBelongsToProfile(scenario.key, profile)),
  });
}

export async function POST(request: Request) {
  let body: { action?: unknown; scenario?: unknown };
  try {
  ...
  if (needsScenario && !scenarioBelongsToProfile(scenario, await demoProfile())) {
    return NextResponse.json({ error: 'scenario is not available for this demo profile' }, { status: 400 });
  }
  ...
  try {
    ...
    const payload = JSON.stringify({ id, action: body.action, scenario, at: Date.now() });
