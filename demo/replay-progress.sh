...
NODES=(
  ...
  "autofactory-flag-testing:Tests:claude-sonnet-4-5-20250929:{\"tests_last_run\":\"pass\"}"
...

{
  ...
  for entry in "${NODES[@]}"; do
    ...
    # handoff checks (packages/shared/src/handoffVerifier.ts) and a sampled
    # LaunchDarkly judge score. Rehearsal is the path used in front of an
    # audience, so it has to show the same evidence a real run does — check and
    # judge names are the real ones, so nothing here contradicts a live run.
    if [[ -z "$SIMULATION_REASON" ]]; then
      case "$key" in
        autofactory-flag-implementer)
          echo "[verify] $key ✓ flag-exists-in-ld: '${SCENARIO}' exists in project '${PROJECT}'"
          echo "[verify] $key ✓ flag-wired-in-code: '${SCENARIO}' referenced in 2 file(s)"
          echo "[judge] $key ← 'autofactory-judge-implementation-quality' score=0.84 — flag is evaluated at the decision point and defaults to control"
          ;;
        autofactory-metrics-author)
          echo "[verify] $key ✓ metric-event-instrumented: event 'checkout-completed' emitted in src/app/api/checkout/route.ts"
          echo "[judge] $key ← 'autofactory-judge-metrics-quality' score=0.79 — conversion and error rate cover the risk this change carries"
          ;;
        autofactory-flag-testing)
          echo "[verify] $key ✓ tests-green-at-handoff: last run_tests execution passed"
          ;;
      esac
    fi
  done
