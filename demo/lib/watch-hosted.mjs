...
function reconcileFromFinalLog() {
  ...
  for (const raw of result.stdout.split(/\r?\n/)) {
    ...
    // The platform's evaluation of an agent's work, and the deterministic gates
    // that re-derive its claims from LaunchDarkly and the checkout. Forwarded so
    // a hosted run shows the same evidence a local one does: without these the
    // pane can say a step completed but not that anything checked it.
    if (
      /\[judge\]\s+autofactory-[a-z0-9-]+\s*←/.test(line) ||
      /^\s*Judge:\s*autofactory-[a-z0-9-]+\s+scored/i.test(line) ||
      /\[verify\]\s+autofactory-[a-z0-9-]+\s+[✓✗]/.test(line) ||
      /Deterministic check failed after/.test(line)
    ) {
      say(line);
      continue;
    }
