...
[[ -n "$BRANCH" ]] || { echo "Scenario '$SCENARIO' has no head branch."; exit 2; }
...
if [[ "${FACTORY_ATTACH:-0}" == "1" ]]; then
  ...
  # time to talk. This removes PR creation and queue time from the audience's
  # wait without pretending the agents are running locally.
  ui_start "trigger" "attaching to the active Actions run"
  if ! RUN=$(G run list --repo "$SLUG" --branch "$BRANCH" --workflow auto-factory.yml --limit 10 \
    --json databaseId,status,conclusion \
    --jq '[.[] | select(.status != "completed" and .conclusion != "skipped")] | .[0].databaseId // empty'); then
    handle_github_failure "finding the active Actions run"
  fi
  if [[ -z "$RUN" ]]; then
    ui_fail "no active run for $BRANCH"
    ui_note "start Live PR once, then choose Attach while it is queued or running"
    exit 1
  fi
  ui_done "attached; no new workflow started"
else
...
# if it were new.
if ! BEFORE=$(G run list --repo "$SLUG" --branch "$BRANCH" --workflow auto-factory.yml --limit 1 \
